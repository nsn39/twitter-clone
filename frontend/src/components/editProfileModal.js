...
function EditProfileModal ({isVisible, onClose}) {
    ...
    return (
        <div onClick={handleClose} id="wrapper" className="fixed inset-0 bg-black bg-opacity-25 font-chirp backdrop-blur-sm flex items-center justify-center">
            <div className="w-11/12 md:w-[45%] flex flex-col bg-white border-b-1 border-black rounded-lg p-4 h-[80vh] overflow-y-scroll">
                ...
                <div className="relative mb-16 mt-4">
                    ...
                    <div className="absolute h-30 w-30 rounded-full border-2 border-white top-[120px] left-[20px]">
                        <img id="profile_pic" src="/lex.jpeg" className="h-28 w-28 rounded-full cursor-pointer fill-black-100" />
                        <div class="absolute rounded-full inset-0 bg-slate-800 opacity-50"></div>
                    </div>

                    <div onMouseOver={onMouseOverPic} onMouseOut={onMouseOutPic} onClick={onPhotoUploadClick} className="absolute top-[160px] left-[55px] flex items-center justify-center h-12 w-12 rounded-full bg-slate-500 opacity-50 hover:bg-slate-200 cursor-pointer">
