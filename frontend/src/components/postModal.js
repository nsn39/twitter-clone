...
function PostModal({isVisible, onClose, activeTweetData, setActiveTweetData}) {
    ...
    const [placeholderText, setPlaceholderText] = useState("What's happening?");
    ...
    useEffect(() => {
        if (activeTweetData) {
            if (activeTweetData.type === "quote") {
                setPlaceholderText("Share your thoughts.");
            }
            else if (activeTweetData.type === "reply") {
                setPlaceholderText("Write your reply.");
            }
        }
        ...
        fetch(REACT_APP_BACKEND_URL + "active_user", {
            ...
            if (data) {
                setActiveUserData({
                    ...
                    "display_picture": data.profile_pic_filename
                });
            }
        })
    }, []);
    ...
    const ReplyTweetComponent = () => {
        if (!activeTweetData) {
            ...
            return (
                <div className="flex flex-row p-3 w-full text-sm md:text-base">
                    <div className="w-[7%] flex flex-col items-center h-full">
                        <img src={activeTweetData.pic} className="flex-none h-10 w-10 rounded-full mb-1" />
                    ...
                    <div className="w-[93%] flex flex-col pl-2 pb-8">
                        <div className="flex flex-row mb-2">
                            ...
                            <p className="flex flex-row text-gray-400 mr-1">
            ...
            return (
                <div className="w-full flex flex-col border-[0.5px] border-slate-400 rounded-2xl p-4">
                    <div className="flex flex-row items-center mb-2">
                        ...
                        <p className="font-bold text-sm md:text-base hover:underline mr-1">
                            <a href="./">{activeTweetData.fullname}</a> 
                        </p>
                        <p className="flex flex-row text-sm md:text-base text-gray-400 mr-1">
                            <p className='mr-1'>&bull;</p>
                            <a href="./">{"@" + activeTweetData.username }</a>
                        </p>
                        <p className='text-gray-400 mr-1'>&bull;</p>
                        <p className='text-gray-400 text-sm md:text-base'>{activeTweetData.datetime}</p>
                    
                    </div>
                    <p className="border-b-200 text-sm md:text-base">{activeTweetData.content}</p>
                </div>
            )
        }
        else {
    ...
    return (
        <div onClick={handleClose} id="wrapper" className="fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm md:flex md:flex-col items-center justify-center pt-4 md:pt-0">
            <div className="md:w-[46%] bg-white flex flex-col rounded-2xl p-2 font-chirp">
                ...
                <div className="flex flex-row px-3 pb-3 items-start border-slate-300">
                    <img src={REACT_APP_FS_URL + activeUserData.display_picture} className='flex-none h-10 w-10 rounded-full mr-4' />

                    <div className='grow flex flex-col items-end'>
                        <textarea className="h-24 w-full mr-2 border-slate-300 outline-none resize-none md:text-xl" type="text" placeholder={placeholderText} onChange={inputHandler} value={inputText}/>

                        <QuoteTweetComponent />
                        <button onClick={tweetSubmitter} className="text-white w-15 md:w-20 font-bold bg-sky-400 mt-2 px-4 py-2 rounded-full">Post</button>
                    </div>
                </div>
            </div>
        </div>
